// Command Openf1CarData-cli is a CLI + REPL for the Openf1CarData SDK,
...
const prompt = "openf1-car-data"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "create_checkout_session endpoint_path_post race_lap subscription_cancel subscription_success token webhook"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      OPENF1_CAR_DATA_APIKEY (api key), OPENF1_CAR_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: OPENF1_CAR_DATA_APIKEY carries the API key and
	// OPENF1_CAR_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("OPENF1_CAR_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("OPENF1_CAR_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewOpenf1CarDataSDK(opts)
