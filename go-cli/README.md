...
boru-driven command-line client **and** interactive REPL for the Openf1CarData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/openf1-car-data-cli)
make build

# 2. See usage (words, entities, env vars)
./openf1-car-data-cli --help
...
OPENF1_CAR_DATA_BASE=https://api.example.com ./openf1-car-data-cli --help

# 6. No arguments -> interactive REPL
./openf1-car-data-cli
openf1-car-data> /help
openf1-car-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/openf1-car-data-cli
   ```
...
Run with no arguments to open a REPL (prompt `openf1-car-data>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `create_checkout_session`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/openf1-car-data-cli`. |
...
(`./openf1-car-data-cli <expr>`) and interactively (the REPL), and expressions compose the
