...
class RaceLapEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return RaceLapEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> RaceLap:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.RaceLap().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: RaceLapCreateData, ctrl=None) -> RaceLap:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
