...
class TestCreateCheckoutSessionEntity:

    def test_should_create_instance(self):
        testsdk = Openf1CarDataSDK.test(None, None)
        ent = testsdk.CreateCheckoutSession(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _create_checkout_session_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["create"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "create_checkout_session." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set OPENF1_CAR_DATA_TEST_CREATE_CHECKOUT_SESSION_ENTID JSON to run live")
        client = setup["client"]

        # CREATE
        create_checkout_session_ref01_ent = client.CreateCheckoutSession(None)
        ...
        create_checkout_session_ref01_data = helpers.to_map(runner.entity_data(create_checkout_session_ref01_ent.create(create_checkout_session_ref01_data, None)))
        assert create_checkout_session_ref01_data is not None



def _create_checkout_session_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/create_checkout_session/CreateCheckoutSessionTestData.json")
    ...
    _entid_env_raw = os.environ.get(
        "OPENF1_CAR_DATA_TEST_CREATE_CHECKOUT_SESSION_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("OPENF1_CAR_DATA_TEST_CREATE_CHECKOUT_SESSION_ENTID"))
