...
class Openf1CarDataTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            Openf1CarDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = Openf1CarDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = Openf1CarDataTestRunner.getenv("OPENF1_CAR_DATA_TEST_LIVE")
        override = Openf1CarDataTestRunner.getenv("OPENF1_CAR_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = Openf1CarDataTestRunner.getenv(key)
        ...
        explain = Openf1CarDataTestRunner.getenv("OPENF1_CAR_DATA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if Openf1CarDataTestRunner._test_control is not None:
            return Openf1CarDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                Openf1CarDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            Openf1CarDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = Openf1CarDataTestRunner.load_test_control()
...
def load_env_local():
    Openf1CarDataTestRunner.load_env_local()


def env_override(m):
    return Openf1CarDataTestRunner.env_override(m)


def entity_data(v):
    return Openf1CarDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return Openf1CarDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return Openf1CarDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return Openf1CarDataTestRunner.load_test_control()


def live_client_options():
    return Openf1CarDataTestRunner.live_client_options()


def live_delay_ms():
    return Openf1CarDataTestRunner.live_delay_ms()
