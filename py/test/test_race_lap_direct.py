...
class TestRaceLapDirect:

    def test_should_direct_load_race_lap(self):
        setup = _race_lap_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-race_lap", "live" if setup["live"] else "unit")
        if _skip:
...
def _race_lap_direct_setup(mockres):
