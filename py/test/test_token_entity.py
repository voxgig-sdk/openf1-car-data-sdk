    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set OPENF_CARDATA_TEST_TOKEN_ENTID JSON to run live")
        client = setup["client"]
        ...
        token_ref01_data_result, err = token_ref01_ent.create(token_ref01_data, None)
        assert err is None
...
def _token_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "OPENF_CARDATA_TEST_TOKEN_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("OPENF_CARDATA_TEST_TOKEN_ENTID"))
