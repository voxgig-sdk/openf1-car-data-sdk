...
[MCP](https://modelcontextprotocol.io) server exposing the Openf1CarData SDK as
two agent tools — `openf1-car-data_list` and `openf1-car-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/openf1-car-data-mcp)
make build

# 2. Provide credentials via the environment
export OPENF1_CAR_DATA_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user openf1-car-data \
  -- /absolute/path/to/openf1-car-data-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./openf1-car-data-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// openf1-car-data_list: first page of records
{ "entity": "create_checkout_session" }
{ "entity": "create_checkout_session", "query": { } }

// openf1-car-data_load: one record by id
{ "entity": "endpoint_path_post", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/openf1-car-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user openf1-car-data \
     -- "$PWD"/dist/*/openf1-car-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `openf1-car-data_list` and `openf1-car-data_load` tools now appear
   in new sessions. Ask the agent to *"list create_checkout_session using openf1-car-data"*
   and it calls `openf1-car-data_list` with `{"entity":"create_checkout_session"}`.
...
```sh
export OPENF1_CAR_DATA_APIKEY=sk_live_xxx            # API key
export OPENF1_CAR_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `openf1-car-data_list` tool
...
### Call the `openf1-car-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `openf1-car-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `openf1-car-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `OPENF1_CAR_DATA_APIKEY` | API key sent with every request. |
| `OPENF1_CAR_DATA_BASE` | Optional override of the API base URL. |

### Entities

The 7 entities valid as the `entity` argument:

create_checkout_session | endpoint_path_post | race_lap | subscription_cancel | subscription_success | token | webhook

### Smoke test via HTTP (raw JSON-RPC)

```sh
./openf1-car-data-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"openf1-car-data_load","arguments":{"entity":"endpoint_path_post","query":{"id":1}}}}'
```
