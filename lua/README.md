...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:CreateCheckoutSession()` — each with the same small set of operations (`load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
-- Create
local created, err = client:CreateCheckoutSession():create({  })
...
```lua
local race_lap, err = client:RaceLap():load()
```
...
```lua
local subscription_cancel, err = client:SubscriptionCancel():load()
```
...
```lua
local subscription_success, err = client:SubscriptionSuccess():load()
```
