...
The Lua SDK for the Openf1CarData API. Provides an entity-oriented interface using Lua conventions.
...
```lua
-- Create
local created, _ = client:CreateCheckoutSession(nil):create({ name = "Example" }, nil)
...
local result, err = client:Openf1CarData(nil):load(
  { id = "test01" }, nil
)
