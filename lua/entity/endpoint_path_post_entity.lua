...
local EndpointPathPostEntity = {}
EndpointPathPostEntity.__index = EndpointPathPostEntity


function EndpointPathPostEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, EndpointPathPostEntity)
  self._name = "endpoint_path_post"
  self._client = client
...
function EndpointPathPostEntity:get_name()
  return self._name
end


function EndpointPathPostEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return EndpointPathPostEntity.new(self._client, opts)
end
...
function EndpointPathPostEntity:mark_deleted()
  self._deleted = true
end


function EndpointPathPostEntity:deleted()
  return true == self._deleted
end


function EndpointPathPostEntity:data_set(args)
  if args ~= nil then
...
function EndpointPathPostEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function EndpointPathPostEntity:match_set(args)
  if args ~= nil then
...
function EndpointPathPostEntity:match_get()
...
function EndpointPathPostEntity:stream(action, args, callopts)
...
function EndpointPathPostEntity:_run_op(ctx, post_done)
  local utility = self._utility
