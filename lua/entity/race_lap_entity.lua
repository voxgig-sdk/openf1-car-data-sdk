...
local RaceLapEntity = {}
RaceLapEntity.__index = RaceLapEntity


function RaceLapEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RaceLapEntity)
  self._name = "race_lap"
  self._client = client
...
function RaceLapEntity:get_name()
  return self._name
end


function RaceLapEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RaceLapEntity.new(self._client, opts)
end
...
function RaceLapEntity:mark_deleted()
  self._deleted = true
end


function RaceLapEntity:deleted()
  return true == self._deleted
end


function RaceLapEntity:data_set(args)
  if args ~= nil then
...
function RaceLapEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RaceLapEntity:match_set(args)
  if args ~= nil then
...
function RaceLapEntity:match_get()
...
function RaceLapEntity:stream(action, args, callopts)
...
function RaceLapEntity:_run_op(ctx, post_done)
  local utility = self._utility
