...
class Openf1CarDataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = Openf1CarDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = Openf1CarDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, Openf1CarDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    Openf1CarDataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    params = Openf1CarDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = Openf1CarDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = Openf1CarDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = Openf1CarDataError.new(
        "graphql_error", "Openf1CarDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = Openf1CarDataSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
