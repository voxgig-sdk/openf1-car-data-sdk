...
class SubscriptionSuccessEntity
  ...
  def make
    opts = @_entopts.dup
    SubscriptionSuccessEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = Openf1CarDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [SubscriptionSuccess, Hash] the current SubscriptionSuccess data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = Openf1CarDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of SubscriptionSuccess fields)
  def match_get
  ...
  #
  # @param reqmatch [SubscriptionSuccessLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.SubscriptionSuccess.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [SubscriptionSuccess, Hash] the loaded SubscriptionSuccess; raises Openf1CarDataError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = Openf1CarDataHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
