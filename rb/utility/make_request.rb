...
module Openf1CarDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = Openf1CarDataResponse.new({})
    ...
    if fetch_err
      ...
      response = Openf1CarDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = Openf1CarDataResponse.new(fetched)
    else
