# Openf1CarData Ruby SDK

The Ruby SDK for the Openf1CarData API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
# Create
created, _ = client.CreateCheckoutSession(nil).create({ "name" => "Example" }, nil)
...
result, err = client.Openf1CarData(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── Openf1CarData_sdk.rb       -- Main SDK module
