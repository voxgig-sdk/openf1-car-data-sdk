...
class SubscriptionCancelEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = subscription_cancel_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "subscription_cancel." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set OPENF1_CAR_DATA_TEST_SUBSCRIPTION_CANCEL_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    subscription_cancel_ref01_data = nil
    if subscription_cancel_ref01_data_raw.length > 0
...
def subscription_cancel_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "subscription_cancel", "SubscriptionCancelTestData.json")
  ...
  entid_env_raw = ENV["OPENF1_CAR_DATA_TEST_SUBSCRIPTION_CANCEL_ENTID"]
