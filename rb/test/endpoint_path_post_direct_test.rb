...
class EndpointPathPostDirectTest < Minitest::Test
  def test_direct_load_endpoint_path_post
    setup = endpoint_path_post_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-endpoint_path_post", setup[:live] ? "live" : "unit")
    if _should_skip
...
def endpoint_path_post_direct_setup(mockres)
