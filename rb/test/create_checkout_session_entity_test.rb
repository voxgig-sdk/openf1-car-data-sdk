...
class CreateCheckoutSessionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set OPENF1_CAR_DATA_TEST_CREATE_CHECKOUT_SESSION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    create_checkout_session_ref01_data = Helpers.to_map(create_checkout_session_ref01_data_result.respond_to?(:data_get) ? create_checkout_session_ref01_data_result.data_get : create_checkout_session_ref01_data_result)
...
def create_checkout_session_basic_setup(extra)
  ...
  entid_env_raw = ENV["OPENF1_CAR_DATA_TEST_CREATE_CHECKOUT_SESSION_ENTID"]
