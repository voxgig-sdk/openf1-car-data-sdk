# RaceLap entity test

require "minitest/autorun"
require "json"
require_relative "../Openf1CarData_sdk"
require_relative "runner"

class RaceLapEntityTest < Minitest::Test
  def test_create_instance
    testsdk = Openf1CarDataSDK.test(nil, nil)
    ent = testsdk.RaceLap(nil)
    assert !ent.nil?
  end

  def test_basic_flow
    setup = race_lap_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create", "load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "race_lap." + _op, _live ? "live" : "unit")
      if _should_skip
        skip(_reason || "skipped via sdk-test-control.json")
        return
      end
    end
    # The basic flow consumes synthetic IDs from the fixture. In live mode
    # without an *_ENTID env override, those IDs hit the live API and 4xx.
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set OPENF1_CAR_DATA_TEST_RACE_LAP_ENTID JSON to run live"
      return
    end
    client = setup[:client]

    # CREATE
    race_lap_ref01_ent = client.RaceLap(nil)
    race_lap_ref01_data = Helpers.to_map(Vs.getprop(
      Vs.getpath(setup[:data], "new.race_lap"), "race_lap_ref01"))

    race_lap_ref01_data_result = race_lap_ref01_ent.create(race_lap_ref01_data, nil)
    race_lap_ref01_data = Helpers.to_map(race_lap_ref01_data_result.respond_to?(:data_get) ? race_lap_ref01_data_result.data_get : race_lap_ref01_data_result)
    assert !race_lap_ref01_data.nil?

    # LOAD
    race_lap_ref01_match_dt0 = {}
    race_lap_ref01_data_dt0_loaded = race_lap_ref01_ent.load(race_lap_ref01_match_dt0, nil)
    assert !race_lap_ref01_data_dt0_loaded.nil?

  end
end

def race_lap_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "race_lap", "RaceLapTestData.json")
  entity_data_source = File.read(entity_data_file)
  entity_data = JSON.parse(entity_data_source)

  options = {}
  options["entity"] = entity_data["existing"]

  client = Openf1CarDataSDK.test(options, extra)

  # Generate idmap via transform.
  idmap = Vs.transform(
    ["race_lap01", "race_lap02", "race_lap03"],
    {
      "`$PACK`" => ["", {
        "`$KEY`" => "`$COPY`",
        "`$VAL`" => ["`$FORMAT`", "upper", "`$COPY`"],
      }],
    }
  )

  # Detect ENTID env override before envOverride consumes it. When live
  # mode is on without a real override, the basic test runs against synthetic
  # IDs from the fixture and 4xx's. Surface this so the test can skip.
  entid_env_raw = ENV["OPENF1_CAR_DATA_TEST_RACE_LAP_ENTID"]
  idmap_overridden = !entid_env_raw.nil? && entid_env_raw.strip.start_with?("{")

  env = Runner.env_override({
    "OPENF1_CAR_DATA_TEST_RACE_LAP_ENTID" => idmap,
    "OPENF1_CAR_DATA_TEST_LIVE" => "FALSE",
    "OPENF1_CAR_DATA_TEST_EXPLAIN" => "FALSE",
  })

  idmap_resolved = Helpers.to_map(
    env["OPENF1_CAR_DATA_TEST_RACE_LAP_ENTID"])
  if idmap_resolved.nil?
    idmap_resolved = Helpers.to_map(idmap)
  end

  if env["OPENF1_CAR_DATA_TEST_LIVE"] == "TRUE"
    merged_opts = Vs.merge([
      {
      },
      extra || {},
    ])
    client = Openf1CarDataSDK.new(Helpers.to_map(merged_opts))
  end

  live = env["OPENF1_CAR_DATA_TEST_LIVE"] == "TRUE"
  {
    client: client,
    data: entity_data,
    idmap: idmap_resolved,
    env: env,
    explain: env["OPENF1_CAR_DATA_TEST_EXPLAIN"] == "TRUE",
    live: live,
    synthetic_only: live && !idmap_overridden,
    now: (Time.now.to_f * 1000).to_i,
  }
end
