...
class SubscriptionSuccessEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set OPENF1_CAR_DATA_TEST_SUBSCRIPTION_SUCCESS_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    subscription_success_ref01_data_dt0_loaded = subscription_success_ref01_ent.load(subscription_success_ref01_match_dt0, nil)
...
def subscription_success_basic_setup(extra)
  ...
  entid_env_raw = ENV["OPENF1_CAR_DATA_TEST_SUBSCRIPTION_SUCCESS_ENTID"]
