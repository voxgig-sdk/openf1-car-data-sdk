...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = Openf1CarDataOmni.make_runner(TEST_JSON_FILE, Openf1CarDataSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    Openf1CarDataError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : Openf1CarDataOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = Openf1CarDataOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
