...
class SubscriptionSuccessEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OPENF1_CAR_DATA_TEST_SUBSCRIPTION_SUCCESS_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function subscription_success_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("OPENF1_CAR_DATA_TEST_SUBSCRIPTION_SUCCESS_ENTID");
