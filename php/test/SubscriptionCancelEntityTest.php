...
class SubscriptionCancelEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = Openf1CarDataSDK::test(null, null);
        $ent = $testsdk->SubscriptionCancel(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = subscription_cancel_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "subscription_cancel." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set OPENF1_CAR_DATA_TEST_SUBSCRIPTION_CANCEL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $subscription_cancel_ref01_data = null;
        if (count($subscription_cancel_ref01_data_raw) > 0) {
...
function subscription_cancel_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/subscription_cancel/SubscriptionCancelTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["subscription_cancel01", "subscription_cancel02", "subscription_cancel03"] as $k) {
    ...
    $entid_env_raw = getenv("OPENF1_CAR_DATA_TEST_SUBSCRIPTION_CANCEL_ENTID");
