...
// Openf1CarData SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class Openf1CarDataMakePoint
{
    public static function call(Openf1CarDataContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof Openf1CarDataError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = Openf1CarDataHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = Openf1CarDataHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
