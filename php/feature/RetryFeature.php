...
// Openf1CarData SDK retry feature
...
class Openf1CarDataRetryFeature extends Openf1CarDataBaseFeature
{
    ...
    public function init(Openf1CarDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (Openf1CarDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(Openf1CarDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(Openf1CarDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
