...
describe('CreateCheckoutSessionEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set OPENF__CAR_DATA_TEST_CREATE_CHECKOUT_SESSION_ENTID JSON to run live')
      return
    }
    ...
    create_checkout_session_ref01_data = await create_checkout_session_ref01_ent.create(create_checkout_session_ref01_data)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['OPENF__CAR_DATA_TEST_CREATE_CHECKOUT_SESSION_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'OPENF__CAR_DATA_TEST_CREATE_CHECKOUT_SESSION_ENTID': idmap,
    'OPENF__CAR_DATA_TEST_LIVE': 'FALSE',
    'OPENF__CAR_DATA_TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['OPENF__CAR_DATA_TEST_CREATE_CHECKOUT_SESSION_ENTID']
