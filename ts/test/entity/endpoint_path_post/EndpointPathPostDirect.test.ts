...
describe('EndpointPathPostDirect', async () => {
  ...
  test('direct-load-endpoint_path_post', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-endpoint_path_post', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['OPENF1_CAR_DATA_TEST_ENDPOINT_PATH_POST_ENTID']
