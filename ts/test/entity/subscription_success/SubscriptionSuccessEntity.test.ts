...
describe('SubscriptionSuccessEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'subscription_success.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set OPENF__CAR_DATA_TEST_SUBSCRIPTION_SUCCESS_ENTID JSON to run live')
      return
    }
    ...
    const subscription_success_ref01_data_dt0 = await subscription_success_ref01_ent.load(subscription_success_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['OPENF__CAR_DATA_TEST_SUBSCRIPTION_SUCCESS_ENTID']
  const idmapOverridden = null != idmapEnvVal && idmapEnvVal.trim().startsWith('{')

  const env = envOverride({
    'OPENF__CAR_DATA_TEST_SUBSCRIPTION_SUCCESS_ENTID': idmap,
    'OPENF__CAR_DATA_TEST_LIVE': 'FALSE',
    'OPENF__CAR_DATA_TEST_EXPLAIN': 'FALSE',
  })

  idmap = env['OPENF__CAR_DATA_TEST_SUBSCRIPTION_SUCCESS_ENTID']
