...
describe('SubscriptionCancelEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'subscription_cancel.' + op, live)) return
    }
    ...
    let subscription_cancel_ref01_data = Object.values(setup.data.existing.subscription_cancel)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/subscription_cancel/SubscriptionCancelTestData.json')
