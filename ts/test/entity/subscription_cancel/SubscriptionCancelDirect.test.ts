...
describe('SubscriptionCancelDirect', async () => {
  ...
  test('direct-load-subscription_cancel', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-subscription_cancel', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['OPENF1_CAR_DATA_TEST_SUBSCRIPTION_CANCEL_ENTID']
