...
describe('RaceLapDirect', async () => {
  ...
  test('direct-load-race_lap', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-race_lap', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['OPENF1_CAR_DATA_TEST_RACE_LAP_ENTID']
