...
//     (\`new Openf1CarDataSDK(...)\` / \`Openf1CarDataSDK.test(...)\` become
//     \`Openf1CarDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'Openf1CarDataSDK'
...
const TEST_SEED = {"entity":{"create_checkout_session":{"test01":{"id":"test01"}},"endpoint_path_post":{"test01":{"id":"test01"}},"race_lap":{"test01":{"id":"test01"}},"subscription_cancel":{"test01":{"id":"test01"}},"subscription_success":{"test01":{"id":"test01"}},"token":{"test01":{"id":"test01"}},"webhook":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new Openf1CarDataSDK(...)\` and any
// \`Openf1CarDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+Openf1CarDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/Openf1CarDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
