...
class Openf1CarDataMakeError
{
    public static function call(?Openf1CarDataContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new Openf1CarDataContext([], null);
        }
        $op = $ctx->op ?? new Openf1CarDataOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new Openf1CarDataResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof Openf1CarDataError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new Openf1CarDataError('', $msg, $ctx);
        ...
        if ($err instanceof Openf1CarDataError) {
