...
//    only - a typed Openf1CarDataContext there would make every ctx
...
final class Openf1CarDataOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new Openf1CarDataContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new Openf1CarDataSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new Openf1CarDataResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new Openf1CarDataError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new Openf1CarDataResponse($clean['response']);
    ...
    public static function err_from_map($m): ?Openf1CarDataError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new Openf1CarDataError(is_string($code) ? $code : '', $msg);
    }
