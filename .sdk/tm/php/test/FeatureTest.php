...
// Openf1CarData SDK feature test
//
...
class FtCtrl extends Openf1CarDataControl
{
...
class FtClient
{
    ...
    private Openf1CarDataUtility $utility;

    public function __construct(Openf1CarDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): Openf1CarDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (Openf1CarDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public Openf1CarDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (Openf1CarDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = Openf1CarDataConfig::make_feature($name);
    ...
    private static function buildUrl(Openf1CarDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof Openf1CarDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new Openf1CarDataResponse($fetched);
        }
    ...
    private function fail(Openf1CarDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (Openf1CarDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
