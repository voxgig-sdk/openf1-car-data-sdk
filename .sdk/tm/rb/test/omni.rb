...
module Openf1CarDataOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if Openf1CarDataOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = Openf1CarDataOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| Openf1CarDataOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || Openf1CarDataOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = Openf1CarDataSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = Openf1CarDataError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { Openf1CarDataOmni.errify(err) }
