...
//
// NewOpenf1CarDataSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.Openf1CarDataSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewOpenf1CarDataSDK(opts)
}
...
func fcCandidates(client *sdk.Openf1CarDataSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.Openf1CarDataSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.Openf1CarDataSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.Openf1CarDataError
