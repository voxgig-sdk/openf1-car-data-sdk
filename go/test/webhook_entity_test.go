...
func TestWebhookEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set OPENF1_CAR_DATA_TEST_WEBHOOK_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func webhookBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OPENF1_CAR_DATA_TEST_WEBHOOK_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OPENF1_CAR_DATA_TEST_WEBHOOK_ENTID"])
