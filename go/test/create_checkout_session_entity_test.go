...
func TestCreateCheckoutSessionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		createCheckoutSessionRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "create_checkout_session"}), "create_checkout_session_ref01"))

		createCheckoutSessionRef01DataResult, err := createCheckoutSessionRef01Ent.Create(createCheckoutSessionRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		createCheckoutSessionRef01Data = core.ToMapAny(entityData(createCheckoutSessionRef01DataResult))
...
func create_checkout_sessionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OPENF1_CAR_DATA_TEST_CREATE_CHECKOUT_SESSION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OPENF1_CAR_DATA_TEST_CREATE_CHECKOUT_SESSION_ENTID"])
