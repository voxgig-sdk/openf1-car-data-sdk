...
func TestTokenEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set OPENF1_CAR_DATA_TEST_TOKEN_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func tokenBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("OPENF1_CAR_DATA_TEST_TOKEN_ENTID")
	...
	idmapResolved := core.ToMapAny(env["OPENF1_CAR_DATA_TEST_TOKEN_ENTID"])
