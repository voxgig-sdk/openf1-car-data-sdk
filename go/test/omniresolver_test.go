...
import (
	...
	omni "github.com/voxgig-sdk/openf1-car-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.Openf1CarDataError); is {
				out := map[string]any{
					"name":    "Openf1CarDataError",
...
func omniCtx(arg any, client *sdk.Openf1CarDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.Openf1CarDataSDK); is {
			client = live
