...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("OPENF1_CAR_DATA_TEST_LIVE") == "TRUE" ||
		os.Getenv("OPENF1_CAR_DATA_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("OPENF1_CAR_DATA_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.Openf1CarDataSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.Openf1CarDataError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.Openf1CarDataSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.Openf1CarDataError{Msg: msg, Code: code}
}
